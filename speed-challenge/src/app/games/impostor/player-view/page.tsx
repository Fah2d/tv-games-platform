...
function PlayerViewContent() {
  ...
  useEffect(() => {
    ...
    socket.on(IMP_STATE, (s: ImpStatePayload) => {
      setState(s)
      // Reset vote tracking when a new round starts
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center" dir="rtl">
        <div className="text-center">
          ...
          <a href="/games/impostor" className="text-purple-400 hover:text-purple-300">عودة</a>
        </div>
      </div>
    )
  }
  ...
  if (phase === 'game_over') {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-white p-6" dir="rtl">
        ...
        <a href="/" className="bg-zinc-700 hover:bg-zinc-600 text-white font-bold px-8 py-3 rounded-xl transition-colors">
          🏠 الرئيسية
        </a>
      </div>
    )
