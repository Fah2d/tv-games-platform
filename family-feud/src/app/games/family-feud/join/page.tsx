...
function JoinContent() {
  ...
  useEffect(() => {
    if (!code || !name) {
      router.replace('/games/family-feud')
      return
    }
  ...
  function joinTeam(teamId: 1 | 2) {
    ...
    socket.once(FF_STATE, (state: RoomStatePayload) => {
      ...
      if (joined) {
        router.replace(
          `/games/family-feud/player-view?code=${code}&name=${encodeURIComponent(name)}&team=${teamId}`
        )
      } else {
        setJoining(null)
      }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center" dir="rtl">
        <div className="text-center">
          ...
          <a href="/games/family-feud" className="text-indigo-400 hover:text-indigo-300">عودة</a>
        </div>
      </div>
    )
  }
