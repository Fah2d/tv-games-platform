...
export default function FamilyFeudEntry() {
  ...
  function goSetup() {
    ...
    router.push(`/games/family-feud/setup?name=${encodeURIComponent(playerName.trim())}`)
  }

  function goJoin() {
    if (!playerName.trim() || roomCode.length < 4) return
    router.push(
      `/games/family-feud/join?code=${roomCode.trim().toUpperCase()}&name=${encodeURIComponent(playerName.trim())}`
    )
  }
