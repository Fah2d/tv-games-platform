  ...
  return (
    <div className="min-h-screen bg-bg-primary flex flex-col items-center justify-center gap-10">
      ...
      <div className="flex flex-col items-center gap-4">
        <p className="text-text-muted text-sm">الجولة {ordinal}</p>
        <p className="text-text-secondary text-lg">انتهت الجولة</p>
        {winner && (
          <p className="text-4xl font-bold" style={{ color: winner.color }}>
            {winner.name}
          </p>
        )}
        <p className="text-text-secondary text-xl">فاز بهذه الجولة</p>
      </div>
      ...
      {/* Next round button */}
      <button
        className="mt-4 px-10 py-4 rounded-xl text-lg font-bold bg-bg-surface text-text-primary border border-border-default hover:border-border-hover transition-colors"
        onClick={onNextRound}
      >
        الجولة التالية
      </button>
