...
export default function RoundIntroScreen({ gameState, onStart }: RoundIntroScreenProps) {
  ...
  const { currentRound, totalRounds, teams } = gameState
  const ordinal = ROUND_ORDINALS[currentRound] ?? toArabicIndic(currentRound)
  const showScores = currentRound > 1
    ...
    >
      <div className="text-center space-y-10">
        ...
        <div className="animate-scale-fade-in">
          ...
          <p className="text-text-muted text-sm mt-2">من {toArabicIndic(totalRounds)} جولات</p>
        </div>
          ...
          <div className="flex items-center gap-12 animate-fade-in-up [animation-delay:600ms]">
            <div className="text-center space-y-2">
              <div className="flex items-center justify-center gap-2">
                <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: team1.color }} />
                <span className="text-text-secondary text-sm">{team1.name}</span>
              </div>
              <div className="text-5xl font-bold" style={{ color: team1.color }}>
                {toArabicIndic(team1.score)}
              </div>
            </div>
            ...
            <div className="text-center space-y-2">
              <div className="flex items-center justify-center gap-2">
                <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: team2.color }} />
                <span className="text-text-secondary text-sm">{team2.name}</span>
              </div>
              <div className="text-5xl font-bold" style={{ color: team2.color }}>
                {toArabicIndic(team2.score)}
              </div>
            </div>
          </div>
        )}
